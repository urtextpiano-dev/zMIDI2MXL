...
pub const Dynamic = enum(u8) {
    ...
    pub fn toMidiValue(self: Dynamic) u8 {
        return DYNAMICS.get(self).midi;
    }

    /// Convert dynamic to MusicXML element name
    pub fn toXmlElement(self: Dynamic) []const u8 {
        return DYNAMICS.get(self).xml;
    }

    /// Get recommended sound dynamics value (0-127) for MusicXML
    pub fn toSoundDynamics(self: Dynamic) u8 {
        return DYNAMICS.get(self).sound;
    }
};

/// Single source of truth for all dynamic mappings
const DynamicInfo = struct {
    midi: u8, // representative MIDI velocity
    sound: u8, // recommended MusicXML <sound dynamics="...">
    xml: []const u8, // MusicXML element/tag ("pp", "mf", etc.)
};

const DynamicsMap = std.EnumArray(Dynamic, DynamicInfo);
const DYNAMICS = DynamicsMap.init(.{
    .ppp = .{ .midi = 8, .sound = 15, .xml = "ppp" },
    .pp = .{ .midi = 24, .sound = 31, .xml = "pp" },
    .p = .{ .midi = 40, .sound = 47, .xml = "p" },
    .mp = .{ .midi = 56, .sound = 63, .xml = "mp" },
    .mf = .{ .midi = 72, .sound = 80, .xml = "mf" }, // MusicXML default kept at 80
    .f = .{ .midi = 88, .sound = 95, .xml = "f" },
    .ff = .{ .midi = 104, .sound = 111, .xml = "ff" },
    .fff = .{ .midi = 120, .sound = 127, .xml = "fff" },
});
...
pub const VelocityStats = struct {
    ...
    pub fn analyze(velocities: []const u8) VelocityStats {
        if (velocities.len == 0) {
        ...
        const n_f = @as(f32, @floatFromInt(velocities.len));
        const mean = @as(f32, @floatFromInt(sum)) / n_f;

        // second pass: variance
        var variance_sum: f32 = 0.0;
        for (velocities) |vel| {
            const x = @as(f32, @floatFromInt(vel));
            const d = x - mean;
            variance_sum += d * d;
        }
        const variance = variance_sum / n_f;
...
pub const DynamicsMapper = struct {
    ...
    pub fn analyzeVelocities(self: *DynamicsMapper, velocities: []const u8) void {
        self.velocity_stats = VelocityStats.analyze(velocities);
    }
    ...
    pub fn mapVelocityToDynamic(self: *DynamicsMapper, velocity: u8, context_velocities: []const u8) Dynamic {
        const normalize = self.config.normalize_to_piece and self.velocity_stats != null;

        // Normalize either the local context average (preferred) or the single value.
        var local_velocity: u8 = blk: {
            if (context_velocities.len > 0) {
                var sum: u32 = 0;
                for (context_velocities) |v| {
                    sum += if (normalize) self.normalizeVelocity(v) else v;
                }
                break :blk @as(u8, @intCast(sum / context_velocities.len));
            } else {
                break :blk (if (normalize) self.normalizeVelocity(velocity) else velocity);
            }
        };

        // Apply sensitivity scaling with explicit clamp (0..127), truncation semantics preserved.
        if (self.config.sensitivity != 1.0) {
            const scaled = @as(f32, @floatFromInt(local_velocity)) * self.config.sensitivity;
            const iv: i32 = @intFromFloat(scaled); // trunc toward zero
            const clamped: i32 = @min(127, @max(0, iv));
            local_velocity = @intCast(clamped); // safe after clamp
        }
    ...
    pub fn processNotes(self: *DynamicsMapper, notes: []const NoteEvent) ![]DynamicMarking {
        ...
        // Pre-analyze velocities if normalization is enabled — streaming, no heap.
        if (self.config.normalize_to_piece) {
            var stats = VelocityStats{
                .min_velocity = 127,
                .max_velocity = 0,
                .mean_velocity = 64.0,
                .std_velocity = 0.0,
            ...
            var sum: u32 = 0;
            for (notes) |n| {
                stats.min_velocity = @min(stats.min_velocity, n.velocity);
                stats.max_velocity = @max(stats.max_velocity, n.velocity);
                sum += n.velocity;
            }

            const n_f = @as(f32, @floatFromInt(notes.len));
            stats.mean_velocity = @as(f32, @floatFromInt(sum)) / n_f;

            // std not needed for normalizeVelocity; leave 0.0 to avoid a second pass
            self.velocity_stats = stats;
        }

        var current_time: u32 = 0;

        // Small stack buffer for 5-note context (i-2..i+2)
        var ctx_buf: [5]u8 = undefined;

        for (notes, 0..) |note, i| {
            current_time += note.delta_time;

            const context_start: usize = if (i >= 2) i - 2 else 0;
            const context_end: usize = @min(notes.len, i + 3);
            const window = notes[context_start..context_end];

            // Fill stack buffer slice using multi-iterator form (item + index)
            var ctx_slice = ctx_buf[0..window.len];
            for (window, 0..) |n, idx| {
                ctx_slice[idx] = n.velocity;
            }

            const new_dynamic = self.mapVelocityToDynamic(note.velocity, ctx_slice);
...
pub fn generateDynamicXml(marking: DynamicMarking, writer: anytype) !void {
    try writer.print(
        ...
        \\  </direction>
        \\ 
    , .{ marking.dynamic.toXmlElement(), marking.dynamic.toSoundDynamics() });
}
