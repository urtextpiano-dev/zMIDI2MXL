...
pub const ArenaAllocator = struct {
    ...
    pub fn reset(self: *ArenaAllocator) void {
        ...
        if (self.logger != null) {
            const current_logger = log_mod.getLogger();
            current_logger.debug("Arena reset: freeing {d} bytes from {d} allocations", .{
        ...
        const reset_time = @as(u64, @intCast(end_time - start_time));
        
        if (self.logger != null) {
            const current_logger = log_mod.getLogger();
            current_logger.debug("Arena reset completed in {d} ns", .{reset_time});
        }
    }
    ...
    pub fn isPerformanceTargetMet(self: *const ArenaAllocator, target_mb: f64) bool {
        const avg_time_ns = self.stats.getAverageAllocationTime();
        const avg_time_ms = avg_time_ns / 1_000_000.0; // Convert ns to ms
        
        // Scale target based on amount of memory
        const mb_processed = @as(f64, @floatFromInt(self.stats.peak_usage)) / (1024.0 * 1024.0);
        const scaled_target_ms = if (mb_processed > 0) (target_mb / mb_processed) * 1.0 else 1.0;
...
pub const EducationalPerformanceMetrics = struct {
    ...
    pub fn recordProcessing(self: *EducationalPerformanceMetrics, notes_count: u64, time_ns: u64) void {
        self.notes_processed += notes_count;
        self.processing_time_per_note_ns = if (self.notes_processed > 0) 
            (self.processing_time_per_note_ns + time_ns) / 2 else time_ns;
...
pub const EducationalArena = struct {
    ...
    pub fn deinit(self: *EducationalArena) void {
        ...
        if (self.base_arena.logger != null) {
            const current_logger = log_mod.getLogger();
            current_logger.debug("Educational arena cleanup: {d} notes processed, {d} cycles completed", .{
                self.metrics.notes_processed,
                self.metrics.successful_cycles,
            });
            
            if (self.metrics.error_count > 0) {
                current_logger.warn("Educational arena had {d} errors during processing", .{self.metrics.error_count});
            }
        }
    ...
    pub fn beginPhase(self: *EducationalArena, phase: EducationalPhase) void {
        ...
        if (self.base_arena.logger != null) {
            const current_logger = log_mod.getLogger();
            current_logger.debug("Educational arena entering phase: {s}", .{phase.toString()});
        }
    }
    ...
    pub fn endPhase(self: *EducationalArena) void {
        if (self.current_phase) |phase| {
            ...
            if (self.base_arena.logger != null) {
                const current_logger = log_mod.getLogger();
                current_logger.debug("Educational arena completing phase: {s}", .{phase.toString()});
            }
        }
    ...
    pub fn processEducationalBatch(self: *EducationalArena, notes_count: u64) !void {
        ...
        defer {
            ...
            self.metrics.recordProcessing(notes_count, processing_time);
        }
        
        // Validate performance requirements during processing
        if (self.metrics.notes_processed > 100) { // Only check after warm-up
            if (!self.metrics.isPerformanceTargetMet()) {
                // Safe logger access - get fresh pointer to avoid use-after-free
                if (self.base_arena.logger != null) {
                    const current_logger = log_mod.getLogger();
                    current_logger.warn("Educational processing exceeding performance target: {d}ns per note", .{
    ...
    pub fn enableErrorRecovery(self: *EducationalArena) void {
        ...
        if (self.base_arena.logger != null) {
            const current_logger = log_mod.getLogger();
            current_logger.info("Educational arena error recovery mode enabled", .{});
        }
    }
    ...
    pub fn disableErrorRecovery(self: *EducationalArena) void {
        ...
        if (self.base_arena.logger != null) {
            const current_logger = log_mod.getLogger();
            current_logger.info("Educational arena error recovery mode disabled", .{});
        }
    }
