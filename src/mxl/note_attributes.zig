...
pub fn getStaffForNote(midi_note: u8) u8 {
    // C4 (60) and above → Treble staff(1), otherwise Bass(2)
    return if (midi_note >= 60) 1 else 2;
}
...
pub fn mapVoiceForPiano(voice: u8, staff: u8) u8 {
    // Normalize: unassigned (0) becomes 1
    const v: u8 = if (voice == 0) 1 else voice;

    if (staff == 2) {
        // Bass staff: 1–4 → 5–8, else default to 5
        return if (v >= 1 and v <= 4) v + 4 else 5;
    } else {
        // Treble (or default): keep 1–4, else clamp to 1
        return if (v >= 1 and v <= 4) v else 1;
    }
}
...
pub const NoteAttributeGenerator = struct {
    ...
    pub fn writeTempoDirection(
        ...
        placement: ?[]const u8, // "above" | "below" | null
    ) !void {
        _ = self;

        // One-liner for optional placement attribute
        const attrs: ?[]const Attribute = if (placement) |p|
            &[_]Attribute{.{ .name = "placement", .value = p }}
        else
            null;

        try xml_writer.startElement("direction", attrs);

        try xml_writer.startElement("direction-type", null);
        try xml_writer.startElement("metronome", null);

        // Beat unit and BPM (one decimal, unchanged)
        ...
        // Emit <sound tempo="..."/> as an empty element (identical semantics)
        try xml_writer.writeEmptyElement("sound", &[_]Attribute{
...
test "NoteAttributeGenerator - write time signature" {
    const allocator = std.testing.allocator;
    var buffer = containers.List(u8).init(allocator);
    defer buffer.deinit();
