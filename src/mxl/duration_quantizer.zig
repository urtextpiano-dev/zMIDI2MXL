...
pub const DurationQuantizer = struct {
    ...
    const Entry = struct { ratio: f64, kind: NoteType };
    const note_table = [_]Entry{
        .{ .ratio = StandardDurations.TWO_FIFTY_SIXTH, .kind = .@"256th" },
        .{ .ratio = StandardDurations.ONE_TWENTY_EIGHTH, .kind = .@"128th" },
        .{ .ratio = StandardDurations.SIXTY_FOURTH, .kind = .@"64th" },
        .{ .ratio = StandardDurations.THIRTY_SECOND, .kind = .@"32nd" },
        .{ .ratio = StandardDurations.SIXTEENTH, .kind = .@"16th" },
        .{ .ratio = StandardDurations.EIGHTH, .kind = .eighth },
        .{ .ratio = StandardDurations.QUARTER, .kind = .quarter },
        .{ .ratio = StandardDurations.HALF, .kind = .half },
        .{ .ratio = StandardDurations.WHOLE, .kind = .whole },
        .{ .ratio = StandardDurations.BREVE, .kind = .breve },
    };
    ...
    pub fn quantizeDuration(self: *const DurationQuantizer, raw_duration: u32) QuantizedDuration {
        ...
        // Pick the closest standard duration and its NoteType in one pass
        var best_ratio: f64 = StandardDurations.QUARTER;
        var best_kind: NoteType = .quarter;
        var min_error: f64 = std.math.inf(f64);

        inline for (note_table) |e| {
            const diff = @abs(raw_ratio - e.ratio);
            const tolerance = e.ratio * QUANTIZATION_TOLERANCE; // expected to be 0.02 (2%)
        ...
        const normalized_float = best_ratio * @as(f64, @floatFromInt(self.normalized_divisions));
        const normalized_duration: u32 = if (normalized_float < 1.0) 1 else @as(u32, @intFromFloat(@round(normalized_float)));
    ...
    pub fn shouldQuantize(self: *const DurationQuantizer, raw_duration: u32) bool {
        ...
        inline for (note_table) |e| {
            const diff = @abs(raw_ratio - e.ratio);
            const tolerance = e.ratio * QUANTIZATION_TOLERANCE; // expected 2%
...
pub const NoteType = enum {
    ...
    /// MusicXML note-type string (exactly the tag name)
