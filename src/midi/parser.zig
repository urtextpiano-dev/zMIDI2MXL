...
pub const NoteDurationTracker = struct {
    ...
    pub fn processNoteOff(self: *NoteDurationTracker, channel: u4, note: u8, off_velocity: u8, tick: u32) std.mem.Allocator.Error!void {
        ...
        if (self.active_notes.fetchRemove(key)) |kv| {
            const note_info = kv.value;
        ...
        // Note: Orphaned Note Off events are silently ignored as per spec
    }
...
pub const TextEvent = struct {
    ...
    pub const TextType = enum(u8) {
        ...
        _,                              // Other text event types
    };
    
    /// Get the text event type if it's a standard one
    ...
    /// Get text event type name as string
...
pub const ControlChangeEvent = struct {
    ...
    /// Get the controller type if it's one we track
...
pub const RpnEvent = struct {
    ...
    pub fn getRpnType(self: RpnEvent) ?RpnType {
        ...
        return RpnType.fromMsbLsb(@intCast(self.parameter >> 7), @intCast(self.parameter & 0x7F));
    }
    
    /// Get parameter name for display
    pub fn getParameterName(self: RpnEvent) []const u8 {
        if (self.is_nrpn) return "NRPN";
        if (self.getRpnType()) |rpn_type| {
            return rpn_type.getName();
        }
        return "Unknown RPN";
    }
    ...
    pub fn getInterpretedValue(self: RpnEvent) f32 {
        if (self.is_nrpn) return @floatFromInt(self.value); // Raw value for NRPN
        
        if (self.getRpnType()) |rpn_type| {
            return switch (rpn_type) {
                .pitch_bend_range => @floatFromInt(self.value >> 7), // Only MSB for semitones
                .fine_tuning => (@as(f32, @floatFromInt(self.value)) - 8192.0) / 81.92, // Cents: (value - 8192) / 81.92
                .coarse_tuning => @as(f32, @floatFromInt(self.value >> 7)) - 64.0, // Semitones: MSB - 64
                else => @floatFromInt(self.value),
            };
        }
...
/// Implements TASK-016 per MIDI_Architecture_Reference.md Section 2.2.3 lines 195-200
fn parsePolyphonicPressureEvent(state: *TrackParserState, result: *TrackParseResult, status_byte: u8) (error_mod.MidiError || std.mem.Allocator.Error)!void {
    // Need 2 data bytes for polyphonic pressure events
    if (state.remainingBytes() < 2) {
        return error_mod.MidiError.UnexpectedEndOfFile;
    }
    
    const note = state.data[state.position];
    const pressure = state.data[state.position + 1];
    state.position += 2;
    
    // Validate note and pressure ranges per MIDI spec
    if (note > 127 or pressure > 127) {
        return error_mod.MidiError.InvalidEventData;
    }
    
    const event = PolyphonicPressureEvent{
        .tick = state.current_tick,
        .channel = extractChannelFromStatus(status_byte),
        .note = @intCast(note),
    ...
    try result.polyphonic_pressure_events.append(event);
}

/// Parse Channel Pressure event and add to result
/// Implements TASK-016 per MIDI_Architecture_Reference.md Section 2.2.6 lines 260-264
fn parseChannelPressureEvent(state: *TrackParserState, result: *TrackParseResult, status_byte: u8) (error_mod.MidiError || std.mem.Allocator.Error)!void {
    // Need 1 data byte for channel pressure events
    if (state.remainingBytes() < 1) {
        return error_mod.MidiError.UnexpectedEndOfFile;
    }
    
    const pressure = state.data[state.position];
    state.position += 1;
    
    // Validate pressure range per MIDI spec
    if (pressure > 127) {
        return error_mod.MidiError.InvalidEventData;
    }
    
    const event = ChannelPressureEvent{
        .tick = state.current_tick,
        .channel = extractChannelFromStatus(status_byte),
        .pressure = @intCast(pressure),
    };
    
    try result.channel_pressure_events.append(event);
}
